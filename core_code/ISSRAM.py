...
if __name__ == '__main__':
    ...
    net = FCGAM(in_dim=384).to(device)

    # 打印网络结构和参数
    summary(net, [(2, 384, 20, 20), (2, 77, 384)])

    inputs = torch.randn(1, 384, 20, 20).cuda()
    text_token =  torch.randn(1, 77, 384).cuda()

    flops, params = profile(net, (inputs, text_token,))
    ...
    def calculate_fps(model, input_size, batch_size=1, num_iterations=100):
        ...
        with torch.no_grad():
            for _ in range(num_iterations):
                ...
                output = model(inputs, text_token)
                # 计算总时间
    ...
    calculate_fps(net, input_size=(), batch_size=2, num_iterations=10)